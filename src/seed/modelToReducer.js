...
export default function modelToReducer(model, initialState) {
  ...
  actionKeys.forEach((actionKey) => {
    ...
    if (actionCfg.exec) {
      ...
      for (let reducerKey in callback) {
        ...
        let type = getActionType(model.displayName, actionKey, reducerKey);
      ...
      let type = getActionType(model.displayName, actionKey);
    ...
    externalReducerKeys.forEach((rk) => {
      ...
      let type = '';
      if (!isAsyncAction) {
        type = getActionType(moduleName, actionName);
        processorMap[type] = subscriptions[rk];
      } else {
        if (typeof subscriptions[rk] === 'function') {
          type = getActionType(moduleName, actionName, statusName);
          processorMap[type] = subscriptions[rk];
          ...
          for (let reducerKey in reducer) {
            if (typeof reducer[reducerKey] !== 'function')
              return;
            type = getActionType(moduleName, actionKey, reducerKey);
  ...
  return function (store = initialState, payload) {
    const processor = processorMap[payload.type] || noop;
    let prevStore = null;
     ...
     */
    let nextStore = processor(prevStore, payload);
