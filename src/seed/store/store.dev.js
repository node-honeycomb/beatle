...
export default function configureStore(initialState = {}, middlewares = [], reducerFactory = () => ({}), callback) {
  ...
  const enhancer = compose(applyMiddleware(thunk, createLogger({level: 'info', collapsed: true}), ...middlewares), window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__ ? window.__REDUX_DEVTOOLS_EXTENSION_COMPOSE__() : (noop) => noop);
