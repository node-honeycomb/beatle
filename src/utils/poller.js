...
export default class Poller {
  ...
  hasTick(res) {
    if (!this._current || this._current._resolved && this._option.checkResult(res) !== false) {
      return true;
    } else {
      this.remove();
      return false;
  ...
  start(promise) {
    ...
    this._interval = setInterval(() => {
      if (this._option.smart) {
        this
          ._current
          .then(() => {
            if (this.hasTick(this._current._result)) {
              this.start();
            }
          });
      } else {
